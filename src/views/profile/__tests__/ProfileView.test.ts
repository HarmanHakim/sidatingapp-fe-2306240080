...
describe('ProfileView', () => {
  ...
  it('should destroy existing DataTable when rebuilding', async () => {
    ...
    // Set the dt property to simulate existing DataTable
    ;(wrapper.vm as any).dt = mockDataTable

    // Call buildDataTable to trigger the destroy logic (lines 15-17)
    ;(wrapper.vm as any).buildDataTable()

    // Should call destroy on existing DataTable
    expect(mockDestroy).toHaveBeenCalled()
    // After destroy, dt should be set to null and then a new DataTable created
    // So we just check that destroy was called, which covers lines 15-17
