...
export const useReplyStore = defineStore('reply', () => {
  ...
  async function updateReply(request: UpdateReplyRequest) {
    loading.value = true;
    error.value = null;
    try {
      const updatedReply = await replyService.updateReply(request);
      const index = replies.value.findIndex(r => r.id === request.id);
      if (index !== -1) {
  ...
  async function deleteReply(request: DeleteReplyRequest) {
    loading.value = true;
    error.value = null;
    try {
      await replyService.deleteReply(request);
      replies.value = replies.value.filter(r => r.id !== request.id);
    } catch (err: any) {
